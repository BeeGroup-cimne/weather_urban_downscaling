...
# Importación de Config: Intentamos importación relativa o absoluta
try:
...
def visualize_results(model, val_ds, title):
    ...
    try:
        ...
        # --- 🛑 FIX: RESOLUCIÓN ROBUSTA DE PATH STATS 🛑 ---
        # Buscar stats en lugares probables
        possible_paths = [
        ...
        stats_path = next((p for p in possible_paths if os.path.exists(p)), None)
        
        if not stats_path:
            print("⚠️ No se encontró stats_config.npz en ubicaciones estándar. Saltando visualización.")
            return
