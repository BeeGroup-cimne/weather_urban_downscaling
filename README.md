...
- lightweight metadata and manifests needed to audit the reported workflow;
...
ERA5-Land forcing data are publicly available from the Copernicus Climate Change Service Climate Data Store. The high-resolution UrbClim target fields used in the study were provided under a restricted data-sharing agreement and are not redistributed in this repository. Weather-station files and heatwave timestamp lists used by the evaluation scripts are generated locally from the documented sources and are not tracked as repository artifacts. Full numerical replication therefore requires access to the same restricted target data and regeneration or provision of the external observational inputs, while the code, configuration, evaluation logic, and reporting pipeline are public.
...
Generated experiment outputs, station CSVs, heatwave timestamp files, checkpoints, and publication figures are intentionally not tracked in Git. They are regenerated by the scripts below or provided through the restricted/local data workflow.
...
```bash
python scripts/tools/derive_aemet_heatwaves.py \
